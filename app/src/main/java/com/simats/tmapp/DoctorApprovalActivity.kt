...
class DoctorApprovalActivity : AppCompatActivity() {
    ...
    private fun renderList(doctors: List<Doctor>) {
        ...
        doctors.forEach { doc ->
            ...
            view.findViewById<View>(R.id.btnApprove).setOnClickListener {
                ...
                Toast.makeText(this, "Optimistic: Approving Dr. ${doc.name ?: (doc.specialization ?: "Doctor")}", Toast.LENGTH_SHORT).show()
            }
            view.findViewById<View>(R.id.btnReject).setOnClickListener {
                viewModel.rejectDoctor(doc)
                Toast.makeText(this, "Optimistic: Rejecting Dr. ${doc.name ?: (doc.specialization ?: "Doctor")}", Toast.LENGTH_SHORT).show()
            }
    ...
    private fun populateCardView(view: View, doc: Doctor) {
        ...
        if (!doc.name.isNullOrEmpty()) {
            tvDoctorName.text = "Dr. ${doc.name}"
        } else {
            tvDoctorName.text = "Unknown Doctor"
        }
        
        val initials = (doc.name ?: "UD").split(" ").mapNotNull { it.firstOrNull()?.uppercase() }.joinToString("").take(2)
        view.findViewById<TextView>(R.id.tvInitials).text = initials
        view.findViewById<TextView>(R.id.tvDoctorSpecialty).text = doc.specialization ?: "General"
        
        val dateStr = "Recent"

        view.findViewById<TextView>(R.id.tvDoctorLic).text = "Lic: MED-889900   •   $dateStr"
