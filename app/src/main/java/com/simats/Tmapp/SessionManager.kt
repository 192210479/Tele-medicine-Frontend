...
class SessionManager(context: Context) {
    ...
    companion object {
        ...
        private const val KEY_DOCTOR_STATUS = "doctor_status"
    ...
    fun saveAvatarVersion(version: Long) {
        prefs.edit().putLong(KEY_AVATAR_VERSION, version).apply()
    }

    fun getAvatarVersion(): Long {
        return prefs.getLong(KEY_AVATAR_VERSION, 0L)
    }

    fun bumpAvatarVersion() {
        saveAvatarVersion(System.currentTimeMillis())
    }
    ...
    fun saveDoctorStatus(status: String?) {
        prefs.edit().putString(KEY_DOCTOR_STATUS, status ?: "Pending").apply()
    }

    fun getDoctorStatus(): String {
        return prefs.getString(KEY_DOCTOR_STATUS, "Pending") ?: "Pending"
    }
    ...
    /**
     * Returns a per-user dark mode SharedPreferences key.
     * Each account (patient/doctor/admin) stores its own preference,
     * keyed by user ID so accounts don't bleed into each other.
     * Falls back to the legacy KEY_THEME when no user is logged in (e.g. on first launch).
     */
    private fun getDarkModeKey(): String {
        val userId = getUserId()
        return if (userId != -1) "dark_mode_$userId" else KEY_THEME
    }

    fun saveDarkMode(isEnabled: Boolean) {
        prefs.edit().putBoolean(getDarkModeKey(), isEnabled).apply()
    }

    fun isDarkMode(): Boolean {
        // Dark mode has been permanently removed. Always return false to avoid crashes 
        // if legacy preferences contain invalid types.
        return false
    }
