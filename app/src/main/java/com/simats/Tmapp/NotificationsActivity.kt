...
class NotificationsActivity : AppCompatActivity() {
    ...
    private fun setupTabs() {
        ...
        val tabs = listOf(tabAll, tabAppointments, tabPrescriptions, tabPayments)
    ...
    private fun filterAndDisplay() {
        val filteredList = if (currentFilter == null) {
            ...
            allNotifications.filter { notif ->
                when (currentFilter) {
                    "Appointment" -> notif.type.contains("Appointment", true)
                    "Prescription" -> notif.type.contains("Prescription", true)
                    "Payment" -> notif.type.contains("Payment", true)
                    else -> notif.type.contains(currentFilter!!, true)
                } || notif.type.contains("Medical Record", true)
            }
        }
        ...
        notifications.forEach { notif ->
            ...
            view.setOnClickListener {
                ...
                if (notif.type.equals("RecordRequest", ignoreCase = true)) {
                    var doctorId = notif.doctorId ?: -1
                    var appointmentId = notif.appointmentId ?: notif.referenceId ?: -1
                    var recordType = "Medical Record"

                    try {
                        if (!notif.data.isNullOrBlank()) {
                            val obj = JSONObject(notif.data!!)
                            doctorId = obj.optInt("doctor_id", doctorId)
                            appointmentId = obj.optInt("appointment_id", appointmentId)
                            recordType = obj.optString("record_type", recordType)
                        }
                    } catch (e: Exception) {
                        Log.e("NotificationsActivity", "Failed to parse RecordRequest data", e)
                    }

                    val intent = Intent(this, ShareMedicalRecordsActivity::class.java)
    ...
    private fun formatTime(timestamp: String): String {
        return TimeUtils.convertUtcToLocal(timestamp)
    }
