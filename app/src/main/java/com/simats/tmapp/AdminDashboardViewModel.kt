...
class AdminDashboardViewModel : ViewModel() {
    ...
    fun loadDashboardData() {
        repository.getDashboardSummary { _summary.postValue(it) }
        
        repository.getRevenueTrend { trend ->
            _revenueTrend.postValue(trend)
            trend?.let {
                if (it.size >= 2) {
                    val current = it.last().amount
                    val previous = it[it.size - 2].amount
                    if (previous > 0) {
                        val growth = ((current - previous) / previous * 100).toInt()
                        _revenueGrowth.postValue("${if (growth >= 0) "+" else ""}$growth%")
                    }
                }
            }
        }
        ...
        repository.getWeeklyAppointments { weekly ->
            ...
            weekly?.let {
                val dayName = SimpleDateFormat("EEE", Locale.ENGLISH).format(Date())
                val today = it.find { day -> day.day.equals(dayName, ignoreCase = true) }?.count ?: 0
                _todayAppointmentsCount.postValue("$today Today")
            }
        }
        ...
        repository.getDoctorActivity { activity ->
            ...
            activity?.let {
                val sum = it.sumOf { day -> day.count }
                _newDoctorsCount.postValue("$sum New")
            }
        }
        ...
        repository.getPatientRegistrations { registrations ->
            ...
            registrations?.let {
                if (it.size >= 2) {
                    val current = it.last().count.toDouble()
                    val previous = it[it.size - 2].count.toDouble()
                    if (previous > 0) {
                        val growth = ((current - previous) / previous * 100).toInt()
                        _patientsGrowth.postValue("${if (growth >= 0) "+" else ""}$growth%")
                    }
                }
            }
        }
